...
@Component({
  selector: 'app-others-activities',
  templateUrl: './others-activities.component.html',
  styleUrls: ['./others-activities.component.scss']
})
export class OthersActivitiesComponent implements OnInit {
  ...
  loadInitActivities() {
    ...
    this.activService.getOthersActivities(firstActivityId).subscribe( result => {
  ...
  loadNextActivities() {
    ...
    this.activService.getOthersActivities(lastActivityId).subscribe( result => {
      const data = (result.body.ocs !== undefined) ? result.body.ocs.data : null;
