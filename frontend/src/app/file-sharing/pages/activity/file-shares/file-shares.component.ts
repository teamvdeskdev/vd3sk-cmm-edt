...
@Component({
  ...
  templateUrl: './file-shares.component.html',
  styleUrls: ['./file-shares.component.scss']
})
export class FileSharesComponent implements OnInit {
  ...
  loadInitActivities() {
    ...
    this.activService.getActivFileShares(firstActivityId).subscribe( result => {
  ...
  loadNextActivities() {
    ...
    this.activService.getActivFileShares(lastActivityId).subscribe( result => {
      const data = (result.body.ocs !== undefined) ? result.body.ocs.data : null;
