...
@Component({
  selector: 'app-act-favorites',
  templateUrl: './act-favorites.component.html',
  styleUrls: ['./act-favorites.component.scss']
})
export class ActFavoritesComponent implements OnInit {
  ...
  loadInitActivities() {
    ...
    this.activService.getActivFavorites(firstActivityId).subscribe( result => {
  ...
  loadNextActivities() {
    ...
    this.activService.getActivFavorites(lastActivityId).subscribe( result => {
      const data = (result.body.ocs !== undefined) ? result.body.ocs.data : null;
