...
import { SamluserDialogcreateComponent } from '../samluser-dialogcreate/samluser-dialogcreate.component';
...
@Component({
  selector: 'app-samluser-disabled',
  templateUrl: './samluser-disabled.component.html',
  styleUrls: ['./samluser-disabled.component.scss']
})

export class SamluserDisabledComponent implements OnInit {
  isdata: boolean = true;
  ...
  ngOnInit() {
    ...
    this.adminSettingsService.userSearchUserList(body).subscribe((result: any) => {
      ...
      this.data = this.utilities.elaborateSamlUserSettings(response, false, false);
      this.getApps();
    });
  }
   ...
   onScroll() {
    ...
    this.adminSettingsService.userSearchUserList(body).subscribe((result: any) => {
        if (result.Dtos.length == 0) {
          ...
          const response = result.Dtos;
          const data = this.utilities.elaborateSamlUserSettings(response, false, false);
  ...
  async enableUser($event: string) {
    if(this.authService.isUserSaml){
      let waitService = await this.adminSettingsService.usersamlEnableUser($event).toPromise();
      if(waitService.Performed){
      ...
      this.adminSettingsService.userEnableUser($event).subscribe((result: any) => {
  ...
  goToEnabledUsers() {
    this.router.navigateByUrl('settings/samlusers-settings');
  }
