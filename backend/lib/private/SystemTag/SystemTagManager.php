...
class SystemTagManager implements ISystemTagManager {
	...
	public function getTagsByIds($tagIds): array {
		...
		$query->select('*')
			...
			->andWhere($query->expr()->eq('user', $query->createParameter('user')))
			->addOrderBy('name', 'ASC')
			->addOrderBy('visibility', 'ASC')
			->addOrderBy('editable', 'ASC')
			->setParameter('tagids', $tagIds, IQueryBuilder::PARAM_INT_ARRAY)
			->setParameter('user', $uidOwner);
	 ...
	 * {@inheritdoc}
	 */
	public function createTag(string $tagName, bool $userVisible, bool $userAssignable,$uidOwner): ISystemTag {
		$query = $this->connection->getQueryBuilder();
		$query->insert(self::TAG_TABLE)
			->values([
				...
				'user' => $query->createNamedParameter($uidOwner),
			]);
