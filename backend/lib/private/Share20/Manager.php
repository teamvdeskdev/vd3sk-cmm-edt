...
class Manager implements IManager {
	...
	public function createShare(\OCP\Share\IShare $share) {
		...
		if ($share->getTarget()!="")
		{
			$target = $this->config->getSystemValue('share_folder', '/') .'/'. $share->getTarget();
		}
		else
		{
				...
				if ($user !== null) {
					...
					if ($emailAddress !== null && $emailAddress !== '') {
						...
						$this->sendMailNotification(
							...
							$share->getLabel().$share->getNode()->getId(),
							$share->getSharedBy(),
							$emailAddress,
	...
	protected function sendMailNotification(IL10N $l,
		...
		$emailTemplate->setSubject($l->t('%1$s shared File or Folder with you »%2$s«', array($initiatorDisplayName, $filename)));
		$emailTemplate->addHeader();
		$emailTemplate->addHeading($l->t('%1$s shared File or Folder with you »%2$s«', [$initiatorDisplayName, $filename]), false);
		//$text = $l->t('%1$s shared with you »%2$s«', [$initiatorDisplayName, $filename]);
