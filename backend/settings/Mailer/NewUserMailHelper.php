...
class NewUserMailHelper {
	...
	public function generateTemplate(IUser $user, $generatePasswordResetToken = false) {
		$userId = $user->getUID();
		$lang = $this->config->getUserValue($userId, 'core', 'lang', 'it');
		...
		$link = $this->config->getSystemValue('frontend.overwrite.url');
    	...
    	if(strpos($this->config->getSystemValue('frontend.overwrite.url'),'tim')!== false )
		{
