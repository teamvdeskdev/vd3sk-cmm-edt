...
class MailQueueHandler {
	...
	protected function sendEmailToUser($userName, $email, $lang, $timezone, $maxTime) {
		...
		$homeLink = '<a href="' . rtrim($this->config->getSystemValue('frontend.overwrite.url'), '/') . '/' . '">' . htmlspecialchars($this->getSenderData('name')) . '</a>';																																		   
		...
		// 	$l->t(' at %s', [$homeLink]),
		// 	$l->t(' at %s', [$this->urlGenerator->getAbsoluteURL('/')])
		// );
		$template->addBodyText(
			$l->t(' at %s', [$homeLink]),
			$l->t(' at %s', [rtrim($this->config->getSystemValue('frontend.overwrite.url'), '/') . '/'])
		);
		if ($skippedCount) {
