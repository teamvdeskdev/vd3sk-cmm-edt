...
class Access extends LDAPUtility {
	...
	const AD_DISABLED_ACCOUNT = [66050,66082,514,546,262658,262690,328194,328226];	
	...
	*/
	...
	public function isAccountDeactivated(string $dn)
	{
		try {
			$attributes = $this->readAttribute($dn,'userAccountControl');		
			if (is_array($attributes) && count($attributes) >= 1) {
				$mainAttribute = (int) $attributes[0];
				if (in_array($mainAttribute,self::AD_DISABLED_ACCOUNT)) {
					return true ;
				}
			}
			return false ;
		} catch (\Throwable $th) {
			\OC::$server->getLogger()->error('[LDAP] Error checking if user is not active : ' . $th->getMessage());
			return false ;
		}
	}
