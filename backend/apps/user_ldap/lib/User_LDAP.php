...
{
	const VDESKCLI_LBL = 'VDESK';
	const DESKTOPCLI_LBL = 'DESKTOP CLIENT';
	const IOS_LBL = 'IOS';
	const ANDR_LBL = 'ANDROID';
	const BRIDGE_LBL = 'BRIDGE';
	...
	/**
	 * checks whether the user is allowed to change his avatar 
	 *
	...
	public function checkPassword($uid, $password)
	{
		...
		if ($user->getUsername() !== false) {
			...
			$hasAppPass = $this->hasValidAppPasswordToken($uid, $password);
			//try by username - this could be useful with some LDAP configs
			if (!$hasAppPass) {
				$hasAppPass = $this->hasValidAppPasswordToken($user->getUsername(), $password);
			}
			if ($hasAppPass) {
				if (true === $this->access->isAccountDeactivated($dn)) {
					\OC::$server->getLogger()->info("[VDESK] AppPassword found but user not active on LDAP AD for  $uid - " . $user->getUsername());
					return false;
				}				
			...
			$ldapCredValid = $this->access->areCredentialsValid($dn, $password);
			if ($ldapCredValid) {
				$this->access->cacheUserExists($user->getUsername());
		...
		\OC::$server->getLogger()->info("[VDESK] No valid credentials found for  $uid  (" . $user->getUsername() . ")");
		return false;
	}
