...
use OCA\Files_Sharing\Service\VdeskPermissionManager;
...
class ShareAPIController extends OCSController
{
	...
	/** @var VdeskPermissionManager  */
	protected $permissionManager = false;
	...
	public function __construct(
		...
		VdeskPermissionManager $permissionManager
	) {
	...
	public function deleteShare(string $id): DataResponse
	{
		...
		try {
			...
			throw new OCSNotFoundException($this->l->t('could not delete share. Share Locked.'));
		}

		if (!$this->canAccessShare($share)) {
			if ($this->permissionManager->getShareSubOwner($share) != $this->currentUser && !$this->customPermissions) {
				throw new OCSNotFoundException($this->l->t('Could not delete share'));
			}
		}

		if (($share->getShareType() === Share::SHARE_TYPE_GROUP || $share->getShareType() === Share::SHARE_TYPE_ROOM) &&
														 
			$share->getShareOwner() !== $this->currentUser &&
			$share->getSharedBy() !== $this->currentUser
		) {
			$this->shareManager->deleteFromSelf($share, $this->currentUser);
		} elseif ($this->permissionManager->getShareSubOwner($share) == $this->currentUser && $this->customPermissions) {
			//unshare from sub owner
			$this->recursiveUnshare($share);
			$this->shareManager->deleteShare($share);
		} else {
			//handle iterative share delete
	...
	private function recursiveUnshare($share)
	{
		try {
			$node = $share->getNode();
			if ($node instanceof \OC\Files\Node\Folder) {
				//remove shares from children
				$nodeChildren = $node->getDirectoryListing();
				foreach ($nodeChildren as $child) {
					if ($child->getType() == 'dir') {
						$childShares = \OC::$server->getShareManager()->getSharesBy($child->getOwner()->getUID(), 0, $child);
						foreach ($childShares as $childShare) {
							if ($childShare->getSharedWith() == $share->getSharedWith()) {
								$this->recursiveUnshare($childShare);
								$this->shareManager->deleteShare($childShare);
							}
						}
					}
				}
	...
	public function createShare(
		...
		if ($this->customPermissions) {
			//set initiator as owner whern plugin's active
			$share->setSharedBy($share->getNode()->getOwner()->getUID());
		}
		...
		try {
			...
			if ($this->customPermissions) {
				if ($share instanceof IShare) {
					$this->permissionManager->setShareSubOwner($share,\OC::$server->getUserSession()->getUser()->getUID());
				}
			}
	...
	public function getShares(
		...
		foreach ($shares as $share) {
			...
			try {
				...
				//if recursive share is active
				$format['sub_owner'] = null ;
				if ($this->customPermissions) {
					$format['sub_owner'] = $this->permissionManager->getShareSubOwner($share);
				}
				$formatted[] = $format;
				if ($share->getSharedBy() === $this->currentUser) {
					$miniFormatted[] = $format;
				}
				//hack for RecurviceShare
				if ($this->customPermissions && $share->getSharedBy() != $this->currentUser) {
					$miniFormatted[] = $format;
				}
	...
	private function shareProviderResharingRights(string $userId, IShare $share, $node): bool
	{
		...
		if (
			$share->getShareType() === \OCP\Share::SHARE_TYPE_CIRCLE && \OC::$server->getAppManager()->isEnabledForUser('circles') &&
			...
			try {
				// $member = 1 //\OCA\Circles\Api\v1\Circles::getMember($sharedWith, $userId, 1);
	...
	/**
	 * Get the value of customPermissions
	 */
	...
	/**
	 * Set the value of customPermissions
	 *
	 * @return bool
	 */
	public function setCustomPermissions(bool $customPermissions)
	{
